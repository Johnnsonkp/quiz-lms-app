...
import { QuizTopic, Subject } from '../../types';
import SubjectCards from '../cards/SubjectCards';
import SingleQuestionComponent from '../quiz/SingleQuestionComponent';
import { loadAllQuizData, transformQuizDataToTopics } from '../../hooks/loadQuiz';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Load quiz topics from individual quiz files
    const loadQuizTopics = async () => {
      try {
        // Load all individual quiz files
        const quizDataArray = await loadAllQuizData();
        
        // Transform the data into structured topics
        const structuredTopics = transformQuizDataToTopics(quizDataArray);
        
        console.log('Loaded quiz topics:', structuredTopics);
        setQuizTopics(structuredTopics);
      } catch (error) {
  ...
  // const loadQuizData  = async (topic: string, subject: string) => {
  //   const data = await import(`../../data/${topic}/${subject}.json`);
  //   return data.default;
  // };
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full">
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 md:p-6 overflow-y-auto border-3 w-[100%]">
        {/* Mobile Header */}
        {isMobile && (
        ...
        {selectedTopic && !selectedSubject && (
          (() => {
            ...
            return (
              <section className="space-y-6">
                <header>
                  <h1 className="text-left !text-lg font-bold text-gray-800">{selectedTopicData.topic}</h1>
